...
4.控制器的`enableEvents`函数启用的不仅是读写事件，还有连接错误事件。因此如果在读写回调结束前不调用
    该函数，则连接无法监听错误事件进而自动关闭
    
5.当控制器的`commitWrite`函数发生错误时,你需要在回调结束前调用`enableEvents`函数以监听错误事件
    用于自动关闭连接。若`enableEvents`也返回错误，可以调用`close()`主动关闭连接
    
6.当触发了读事件而未设置读回调时，如果设置了写回调则默认启用写事件（如果写回调也未设置则关闭连接）。
    触发写事件而未设置写回调时同理。
    
7.控制器的函数不允许在单次回调调用`enableEvents`后调用
